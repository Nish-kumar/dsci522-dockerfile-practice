...
RUN conda-lock install -n 522_docker_env conda-lock.yml

# make dockerlock the default environment 
RUN echo "source /opt/conda/etc/profile.d/conda.sh && conda activate 522_docker_env" >> ~/.bashrc
...
CMD ["/bin/bash", "-c", "source /opt/conda/etc/profile.d/conda.sh && conda activate 522_docker_env && jupyter lab --ip=0.0.0.0 --port=8888 --no-browser --allow-root --IdentityProvider.token='' --ServerApp.password=''"]
